...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  try {
    if (useMockDB) {
      const patient = mockDB.getPatientById(params.id);
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (useMockDB) {
      const updated = mockDB.updatePatient(params.id, updateData);
    ...
    try {
      ...
      const updated = await Patient.findByIdAndUpdate(params.id, { $set: updateData }, { new: true, runValidators: true });
      if (!updated) return NextResponse.json({ error: 'Patient not found' }, { status: 404 });
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (useMockDB) {
      const deleted = mockDB.deletePatient(params.id);
